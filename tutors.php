<?php include 'php/header.php';?>

<div class="wrapper">
  <div class="page-header">
    <h1 class="page-title">Meet the Best Tutors in Birmingham, AL</h1>
  </div>
  <div class="content">
    <div class="main-column-tutors">
      <div class="flexcontainer1">
        <div class="tutorselect">
            <select class="classes">
                <option selected value="classes">Select a SUBJECT</option>
                <optgroup label="Math">
                    <option value="elementary-math">Elementary Math</option>
                    <option value="pre-algebra">Pre-Algebra</option>
                    <option value="algebra-one">Algebra 1</option>
                    <option value="algebra-two">Algebra 2</option>
                    <option value="geometry">Geometry</option>
                    <option value="pre-calculus">Pre-Calculus</option>
                    <option value="calculus">Calculus</option>
                    <option value="ap-cal-ab">AP Calculus AB</option>
                    <option value="ap-cal-bc">AP Calculus BC</option>
                    <option value="statistics">Statistics</option>
                    <option value="ap-statistics">AP Statistics</option>
                  </optgroup>
                  <optgroup label="Science">
                      <option value="physical-science">Physical Science</option>
                      <option value="biology">Biology</option>
                      <option value="ap-bio">AP Biology</option>
                      <option value="chemistry">Chemistry</option>
                      <option value="ap-chemistry">AP Chemistry</option>
                      <option value="physics">Physics</option>
                      <option value="ap-physics-1">AP Physics 1: Algebra-Based</option>
                      <option value="ap-physics-2">AP Physics 2: Algebra-Based</option>
                      <option value="ap-physics-c1">AP Physics C: Electricity & Magnetism</option>
                      <option value="ap-physics-c2">AP Physics C: Mechanics</option>
                      <option value="ap-environmental-science">AP Environmental Science</option>
                  </optgroup>
                  <optgroup label="Test Prep">
                      <option value="act">ACT Prep</option>
                      <option value="test-prep">Test Prep (non-ACT)</option>
                      <option value="homework-help">Homework Help</option>
                  </optgroup>
                  <optgroup label="English">
                      <option value="reading">Reading</option>
                      <option value="english">English</option>
                      <option value="ap-english-lang">AP English Language and Composition</option>
                      <option value="ap-english-lit">AP English Literature and Composition</option>
                  </optgroup>
                  <optgroup label="Social Science">
                      <option value="history">History</option>
                      <option value="ap-us-history">AP U.S. History</option>
                      <option value="ap-euro-history">AP European History</option>
                      <option value="ap-world-history">AP World History</option>
                      <option value="government-economics">Government/Economics</option>
                      <option value="ap-us-government">AP U.S. Government & Politics</option>
                      <option value="ap-comp-government">AP Comparative Government & Politics</option>
                      <option value="ap-macro">AP Macroeconomics</option>
                      <option value="ap-micro">AP Microeconomics</option>
                      <option value="ap-psych">AP Psychology</option>
                      <option value="ap-compsci-a">AP Computer Science A</option>
                  </optgroup>
                   <optgroup label="Foreign Languages">
                       <option value="german">German</option> 
                       <option value="spanish">Spanish</option>
                       <option value="french">French</option>
                       <option value="ap-spanish">AP Spanish Language & Culture</option>

                  </optgroup>
                  <optgroup label="Other">
                       <option value="ap-research">AP Research</option> 
                  </optgroup>
                </select>
              </div>

              <div class="tutorselect">
              <select class="classes">
                    <option option selected value="classes">Select a MEETING PLACE</option>
                    <option value="brook">Brookwood Village O'Henry's Coffee</option>
                    <option value="cahasta">Cahaba Heights Starbucks</option>
                    <option value="emlib">Emmet O'Neal Library</option>
                    <option value="hwlib">Homewood Library</option>
                    <option value="hoolib">Hoover Library</option>
                    <option value="bam">Hwy. 280 Books-a-Million</option>
                    <option value="280stae">Hwy. 280 Starbucks (Eastbound)</option>
                    <option value="280staw">Hwy. 280 Starbucks (Westbound)</option>
                    <option value="home">In-Home (address inside tutor's service area)</option>
                    <option value="chur">Mountain Brook Church Street Coffee and Books</option>
                    <option value="mbsta">Mountain Brook Starbucks</option>
                    <option value="nslib">North Shelby Library</option>
                    <option value="samlib">Samford University Library</option>
                    <option value="skype">Skype</option>
                    <option value="barn">Summit Barnes and Noble Coffee Shop</option>
                    <option value="vlib">Vestavia Hills Public Library</option>
                    <option value="vsta">Vestavia Starbucks</option>
                    <option value="zoom">Zoom</option>
                    <option value="other">Other</option>
                  </select>
                </div>

                <div class="tutorselect">
                <select class="classes">
                    <option selected value="classes">Select a PRICE RANGE</option>
                    <option value="30s">$30-$39/hour</option>
                    <option value="40s">$40-$49/hour</option>
                    <option value="50s">$50-$59/hour</option>
                    <option value="60s">$60/hour</option>
                  </select>
              </div>

              <div class="tutorselect">
                <select class="classes">
                    <option selected value="classes">Select GENDER</option>
                    <option value="ma">Male</option>
                    <option value="fe">Female</option>
                  </select>
              </div>
            </div>
            <div class="tutorselect">
                  <input type="text" class="zipcode" name="zipcode" placeholder="Select a ZIP CODE">
                </div>

<div class="flexcontainer2">

          <!--FAHAD

            <div class="ma random classes elementary-math pre-algebra algebra-one algebra-two pre-calculus calculus ap-cal-ab ap-cal-bc chemistry ap-chemistry physics ap-physics-1 ap-physics-2 ap-physics-c2 test-prep homework-help english skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 50s"><span class="hide">35244,35142,35226,35022,35020,35021,35221,35080,35211,35228,35124,35216,35144,35114,35064,35209,35061,35229,35208,35243,35137,35007,35218,35205,35127,35254,35233,35204,35224,35242,35238,35203,35283,35282,35266,35261,35260,35259,35255,35253,35295,35287,35296,35294,35292,35291,35290,35297,35293,35298,35288,35249,35246,35201,35202,35219,35220,35231,35232,35236,35237,35285,35043,35111,35223,35222,35213,35234,35023,35060,35207,35214,35212,35118,35074,35115,35005,35206,35051,35210,35068,35440,35187,35217,35040,35181</span>
               <a href="/fahad"><img class="tutor" src="images/fahad-tutor-birmingham-al.jpg" alt="fahad tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Fahad</tutorname> is a sophomore at UAB majoring in Biophysics and minoring in Mathematics/Chemistry... <a href="/fahad" target="_blank">Read More.</a> <b>$50/hour</b><br><br><br></p>
                </span>
            </div>-->

          <!--CARSON

            <div class="fe random classes elementary-math pre-algebra algebra-one algebra-two pre-calculus calculus statistics physical-science biology ap-bio chemistry ap-chemistry physics ap-physics-1 ap-physics-2 test-prep homework-help english ap-english-lang ap-english-lit skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 30s"><span class="hide">35205,35233,35203,35231,35220,35219,35202,35201,35283,35285,35232,35236,35266,35261,35260,35259,35255,35253,35282,35246,35237,35287,35294,35295,35297,35293,35292,35291,35298,35290,35288,35296,35249,35238,35209,35229,35204,35222,35234,35254,35213,35223,35208,35211,35207,35212,35243,35218,35216,35221,35206,35228,35064,35226,35214,35142,35068,35217,35224,35210,35060,35061,35127,35242,35181,35244,35119,35020,35021,35015,35036,35117,35215</span>
                <a href="/carson"><img class="tutor" src="images/carson-tutor-birmingham-al.jpg" alt="carson tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Carson</tutorname> is a senior at UAB majoring in biology with a double minor in chemistry and business administration.... <a href="/carson" target="_blank">Read More.</a> <b>$38/hour</b><br><br><br></p>
            </span>
            </div>-->

          <!--HEATHER-->

            <div class="fe random classes elementary-math reading english homework-help pre-algebra skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 50s"><span class="hide">35242,35243,35216,35043,35124,35244,35229,35223,35226,35209,35238,35213,35205,35142,35147,35211,35222,35233,35185,35201,35202,35219,35220,35231,35232,35236,35237,35246,35249,35253,35255,35259,35260,35261,35266,35282,35283,35285,35287,35288,35290,35291,35292,35293,35294,35295,35296,35297,35298,35203,35210,35212,35094,35144,35234,35221,35204,35254,35206,35007,35208,35051,35137,35228,35020,35218,35176,35015,35064,35021,35207,35186,35022,35078,35080,35061,35217,35114,35224,35068,35127,35235,35214,35060,35215,35178,35119,35040,35004,35173,35181,35044,35036,35117,35005,35115,35118,35052,35023,35048,35143,35111,35032,35128,35187,35116,35054,35071,35126,35123,35112,35073</span>
                <a href="/heather"><img class="tutor" src="images/heather-tutor-birmingham-al.jpg" alt="heather tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Heather</tutorname> has been teaching kindergarten through 6th grade for over 15 years... <a href="/heather" target="_blank">Read More.</a> <b>$55/hour</b><br><br><br></p>
            </span>
            </div>

          <!--FAITH-->

              <div class="fe random classes ap-us-government ap-us-history ap-world-history ap-english-lang ap-english-lit ap-environmental-science ap-bio algebra-one algebra-two biology chemistry elementary-math english government-economics history homework-help physical-science pre-algebra test-prep skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 40s"><span class="hide">35213,35223,35212,35222,35238,35206,35210,35201,35202,35219,35220,35231,35232,35236,35237,35246,35249,35253,35255,35259,35260,35261,35266,35282,35283,35285,35287,35288,35290,35291,35292,35293,35294,35295,35296,35297,35298,35205,35233,35234,35203,35243,35229,35209,35015,35217,35204,35207,35216,35254,35094,35242,35235,35068,35211,35215,35208,35119,35218,35226,35214,35221,35142,35228,35147,35181,35064,35173,35060,35244,35224,35176,35117,35036,35061,35043,35004,35124,35048,35127,35116,35020,35126,35123,35071,35021,35185,35005,35112,35073,35118,35022,35091,35052,35139,35144,35178</span>
                  <a href="/faith"><img class="tutor" src="images/faith-tutor-birmingham-al.jpeg" alt="faith tutor in birmingham" /></a>
                  <span class="biowrapper">
                  <p class="bio"><br><tutorname>Faith</tutorname> is a senior at UAB majoring in Biomedical Sciences.... <a href="/faith" target="_blank">Read More.</a> <b>$40/hour</b><br><br><br></p>
              </span>
              </div>

            <!--KIM

            <div class="fe random classes act ap-comp-government ap-english-lang ap-english-lit ap-euro-history ap-german ap-geo ap-us-government ap-us-history ap-world-history elementary-math algebra-one algebra-two english geometry government-economics history homework-help pre-algebra test-prep german skype zoom 50s"><span class="hide"></span>
                <a href="/kim"><img class="tutor" src="images/kim-tutor-birmingham-al.jpg" alt="kim tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Kim</tutorname> is a certified teacher in math, German, and history... <a href="/kim" target="_blank">Read More.</a> <b>$50/hour</b><br><br><br></p>
            </span>
            </div>-->

            <!--OLIVIA-->

            <div class="fe random classes act elementary-math algebra-one algebra-two biology calculus chemistry english homework-help pre-algebra pre-calculus test-prep skype zoom brook cahasta emlib hwlib hoolib home chur mbsta samlib barn vlib vsta other 40s"><span class="hide">35233,35203,35287,35285,35283,35282,35266,35261,35260,35288,35290,35291,35292,35293,35294,35295,35296,35297,35298,35259,35255,35219,35201,35202,35220,35231,35232,35236,35237,35246,35249,35253,35205,35234,35204,35222,35238,35254,35209,35229,35213,35207,35208,35223,35212,35211</span>
                <a href="/olivia"><img class="tutor" src="images/olivia-tutor-birmingham-al.jpg" alt="olivia tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Olivia</tutorname> is a junior at UAB majoring in Biology and minoring in Psychology... <a href="/olivia" target="_blank">Read More.</a> <b>$40/hour</b><br><br><br></p>
            </span>
        </div>

            <!--MANUELA-->

            <div class="fe random classes act elementary-math pre-algebra algebra-one algebra-two statistics ap-statistics physical-science biology ap-bio chemistry ap-chemistry physics ap-physics-1 ap-physics-2 ap-environmental-science test-prep homework-help reading english ap-english-lang ap-english-lit ap-psych spanish ap-spanish skype zoom brook cahasta emlib hwlib hoolib home chur mbsta samlib barn vlib vsta other 50s"><span class="hide">35209,35211,35229,35205,35233,35208,35254,35221,35204,35203,35216,35298,35266,35282,35283,35285,35287,35288,35290,35291,35292,35293,35294,35295,35296,35297,35261,35260,35201,35202,35219,35220,35231,35232,35236,35237,35246,35249,35253,35255,35259,35226,35238,35142,35218,35228,35243,35064,35222,35234</span>
                <a href="/manuela"><img class="tutor" src="images/manuela-tutor-birmingham-al.jpg" alt="manuela tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Manuela</tutorname> is working on a BS in Genetics and Genomic Sciences and minors in Biology and Chemistry at UAB... <a href="/manuela" target="_blank">Read More.</a> <b>$50/hour</b><br><br><br></p>
            </span>
        </div>

            <!--STANLEY

            <div class="ma random classes elementary-math pre-algebra algebra-one algebra-two geometry pre-calculus calculus statistics chemistry ap-chemistry physics act test-prep homework-help reading english ap-english-lang ap-english-lit government-economics ap-micro ap-macro french skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 40s"><span class="hide">35242,35043,35185,35147,35243,35124,35216,35051,35186,35223,35244,35094,35078,35176,35229,35213,35238,35209,35226,35210,35205,35007,35222,35144,35137,35142,35233,35212,35178,35211,35298,35297,35296,35295,35294,35293,35292,35291,35249,35246,35237,35236,35232,35231,35220,35202,35201,35219,35253,35255,35259,35290,35288,35287,35285,35283,35282,35260,35261,35266,35206,35203,35015</span>
               <a href="/stanley"><img class="tutor" src="images/stanley-tutor-birmingham-al.jpg" alt="stanley tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Stanley</tutorname> is a senior at the University of Chicago majoring in Economics... <a href="/stanley" target="_blank">Read More.</a> <b>$40/hour</b><br><br><br></p>
                </span>
            </div>-->

            <!--KIRSTIN-->

            <div class="fe random classes act ap-bio algebra-one algebra-two biology homework-help pre-algebra pre-calculus test-prep skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 50s"><span class="hide">35243,35242,35223,35216,35229,35238,35213,35209,35205,35222,35233,35201,35202,35219,35220,35231,35232,35236,35237,35246,35249,35260,35255,35259,35253,35266,35298,35261,35297,35296,35295,35294,35293,35292,35291,35290,35288,35287,35285,35283,35282,35203,35226,35210,35212,35211,35234,35244,35206,35204,35142,35254,35043,35124,35094,35208,35147,35221,35015,35207,35218,35228,35217,35064,35185,35020,35176,35068,35235,35021,35214,35061,35224,35215,35119,35144,35060</span>
                <a href="/kirstin"><img class="tutor" src="images/kirstin-tutor-birmingham-al.jpg" alt="kirstin tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Kirstin</tutorname> graduated from UAB with a Masters in Clinical Laboratory Science... <a href="/kirstin" target="_blank">Read More.</a> <b>$50/hour</b><br><br><br></p>
            </span>
            </div>

            <!--ANNA-->

            <div class="fe random classes ap-bio ap-english-lang ap-english-lit ap-environmental-science ap-psych elementary-math act biology english homework-help pre-algebra test-prep skype zoom 30s"><span class="hide"></span>
                <a href="/anna"><img class="tutor" src="images/anna-tutor-birmingham-al.png" alt="anna tutor in birmingham" /></a>
                <span class="=biowrapper">
                <p class="bio"><br><tutorname>Anna</tutorname> is a sophomore at UAB majoring in Neuroscience... <a href="/anna" target="_blank">Read More.</a> <b>$35/hour</b><br><br><br></p>
            </span>
            </div>

            <!--LINDSAY-->

            <div class="fe random classes act test-prep homework-help skype zoom brook cahasta emlib hwlib hoolib bam 280stae 280staw home chur mbsta nslib samlib barn vlib vsta other 50s"><span class="hide">35226,35142,35216,35211,35209,35229,35221,35244,35205,35228,35243,35208,35233,35020,35254,35064,35238,35203,35218,35204,35021,35201,35202,35219,35220,35231,35232,35236,35237,35246,35249,35253,35255,35259,35260,35261,35266,35282,35283,35285,35287,35288,35290,35291,35292,35293,35294,35295,35296,35297,35298,35223,35061,35222,35213,35234,35124,35242,35224,35022,35127,35207,35212,35060,35214,35043,35144,35206,35210,35080,35068,35217,35114,35007,35137,35118,35181,35005,35023,35015,35036,35119,35094,35117,35147,35111,35215,35235,35185,35051,35073,35176,35071</span>
                <a href="/lindsay"><img class="tutor" src="images/lindsay-tutor-birmingham-al.jpg" alt="lindsay tutor in birmingham" /></a>
                <span class="biowrapper">
                <p class="bio"><br><tutorname>Lindsay</tutorname> graduated from the University of Alabama with a B.S. in Elementary Education... <a href="/lindsay" target="_blank">Read More.</a> <b>$55/hour</b><br><br></p>
            </span>
        </div>
</div>
            <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
            <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

            <!--1 shows/hides tutors based on select box-->
            <!--2 makes profile divs clickable-->
            <!--3 shows/hides tutors based on input box and tutor classes-->
            <!--TODO: get 1 and 3 to work together: https://stackoverflow.com/questions/28887410/using-jquery-to-filtering-with-two-input-types-->

            <script>

              $("select.classes").change(function(){
                var filters = $.map($("select.classes").toArray(), function(e){
                    return $(e).val();
                }).join(".");
                $("div.flexcontainer2").find("div").hide();
                $("div.flexcontainer2").find("div." + filters).show();
              });

              $(".random").click(function() {
                window.location = $(this).find("a").attr("href"); 
                return false;
              });

              $(document).ready(function(){
                $(".zipcode").on("keyup", function() {
                  var value = $(this).val().toLowerCase();
                  $(".flexcontainer2 div").filter(function() {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
                  });
                });
              });
            </script>
        </div>    
    </div>
</div>

<?php include 'php/footer.php';?>